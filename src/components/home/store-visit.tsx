...
export function StoreVisit({ dict }: { dict: Dictionary }) {
  ...
  return (
    <section className="bg-linen-soft px-[var(--spacing-gutter)] py-[var(--spacing-section)] text-ink">
      <div className="mx-auto grid max-w-[1100px] items-center gap-[clamp(36px,6vw,96px)] md:grid-cols-2">
        ...
        <Reveal className="max-w-[420px]">
          <div className="text-[12px] tracking-[0.06em] text-graphite uppercase">{store.eyebrow}</div>
          <h2 className="mb-5 mt-4 font-sans text-[clamp(20px,2vw,24px)] font-medium tracking-[0.02em] uppercase">
            {store.title}
          </h2>
          <p className="m-0 text-[15px] leading-relaxed text-graphite">{store.text}</p>
          <address className="mt-5 text-[15px] not-italic leading-relaxed text-graphite">
            {dict.footer.address}
            <br />
            {dict.footer.city}
          </address>
