  ...
  return (
    <>
      <button
        ...
        className="cursor-pointer border border-ink/70 px-6 py-3 text-[13px] font-medium tracking-[0.06em] text-ink uppercase transition-colors hover:bg-ink hover:text-linen"
      >
        {label}
      </button>
      ...
      <dialog
        ...
        className="m-auto w-[min(960px,92vw)] bg-transparent p-0 backdrop:bg-carbon/80"
      >
        <div className="flex justify-end pb-3">
          <button
            type="button"
            onClick={close}
            className="cursor-pointer font-mono text-xs tracking-[0.14em] text-linen uppercase hover:text-bronze"
          >
            {closeLabel} ✕
          </button>
        </div>
        {videoSrc ? (
          <video src={videoSrc} controls playsInline className="block aspect-video w-full bg-carbon" />
        ) : (
