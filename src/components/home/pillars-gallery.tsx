...
export function PillarsGallery({ dict, locale }: { dict: Dictionary; locale: Locale }) {
  return (
    <section aria-label={dict.gallery.ariaLabel} className="bg-carbon">
        ...
        {dict.gallery.items.map((item, index) => {
          ...
          return (
            ...
            >
              ...
              <span
                aria-hidden="true"
                className="absolute inset-0 bg-linear-to-t from-carbon/70 via-carbon/15 to-transparent"
              />

              <div className="absolute inset-x-0 bottom-0 flex flex-col items-center px-6 pb-[clamp(28px,4vw,44px)] text-center">
                <h3 className="m-0 max-w-[260px] text-[15px] font-normal leading-snug tracking-[0.04em] text-linen uppercase">
                  {item.title}
                </h3>
                <Link
                  href={routes.about(locale, item.section)}
                  className="mt-5 border border-linen/85 px-5 py-2.5 text-[11px] tracking-[0.06em] text-linen uppercase transition-colors hover:bg-linen hover:text-ink"
                >
                  {item.cta}
                </Link>
              </div>
